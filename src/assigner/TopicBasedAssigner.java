...
public class TopicBasedAssigner {
	//Platform = UI, SWT (Component)
		...
		public static void main(String[] args) throws Exception{
			...
			String project = projects[4];
			Class.forName("org.h2.Driver");
			if(projects[4].split("/").length>2)
				project = projects[4].split("/")[0]+"/"+projects[4].split("/")[1];

			Connection conn = DriverManager.getConnection("jdbc:h2:./DB/"+projects,"sa","");
			
			File directory = new File("./data/forTopic/"+projects[4]);
			if(!directory.exists())
				directory.mkdirs();
			directory = new File("./data/forTopic/"+projects[4]+"/test");
			if(!directory.exists())
				directory.mkdirs();			
			
			int total = 0;
			String sql = "select count(*) as num from bug_report a, meta_field b where a.bug_id = b.bug_id order by bug_id asc";
			Statement q = dbConn.createStatement();
			ResultSet rs = q.executeQuery(sql);
			if(rs.next()) total = rs.getInt("NUM");			
			
			sql = "select * from bug_report where a.bug_id = b.bug_id order by bug_id asc";
			q = dbConn.createStatement();
			rs = q.executeQuery(sql);
			int num = 0;
			while(rs.next()){
